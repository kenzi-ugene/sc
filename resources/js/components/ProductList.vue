    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <div v-if="showForm" class="mb-6 bg-gray-100 p-4 rounded">
        <form @submit.prevent="addProduct">
          <input v-model="newProduct.name" type="text" placeholder="Name" class="mb-2 w-full border p-2" required />
          <textarea v-model="newProduct.description" placeholder="Description" class="mb-2 w-full border p-2" required></textarea>
          <input v-model.number="newProduct.price" type="number" placeholder="Price" class="mb-2 w-full border p-2" required />
          <input v-model.number="newProduct.stock" type="number" placeholder="Stock" class="mb-2 w-full border p-2" required />
          <input @change="onFileChange" type="file" class="mb-2 w-full" />
      ...
      <div v-if="editingProduct" class="mb-6 bg-gray-100 p-4 rounded">
        ...
        <form @submit.prevent="updateProduct">
          <input v-model="editForm.name" type="text" placeholder="Name" class="mb-2 w-full border p-2" required />
          <textarea v-model="editForm.description" placeholder="Description" class="mb-2 w-full border p-2" required></textarea>
          <input v-model.number="editForm.price" type="number" placeholder="Price" class="mb-2 w-full border p-2" required />
          <input v-model.number="editForm.stock" type="number" placeholder="Stock" class="mb-2 w-full border p-2" required />
          <input @change="onEditFileChange" type="file" class="mb-2 w-full" />
          <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded">Update</button>
          <button type="button" @click="cancelEdit" class="ml-2 bg-gray-400 text-white px-4 py-2 rounded">Cancel</button>
        </form>
      </div>
